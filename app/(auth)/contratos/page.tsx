...
const bancos = async () => {
  try {
    ...
    if (response.ok) {
      ...
      data.forEach(dato => {
        if (!select.querySelector(`option[value="${dato.id_banco}"]`)) {
          let option = document.createElement('option');
          option.value = dato.id_banco;
...
const searchContrato = async () => {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      if (data && data.data) {
        const {id, email, identificacion, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, direccion, telefono, nro_cuenta, tipo_cuenta, contratos } = data.data;
        ...
        if (contratos && contratos.length > 0) {
          ...
          showMessage({ title: 'Advertencia', cuerpo: 'No se encontró el contrato pero sí el contratista.' });
        }
      } else {
